...
class DualNet(object):
    def __init__(self, sess, image_size=128, batch_size=1,fcn_filter_dim = 64,  \
                 ...
                 checkpoint_dir=None, lambda_A = 100, lambda_B = 100, lambda_pair=100,\
        ...
        self.dir_name ='l1-DualGAN'
        '''"%s-batch_sz_%s-img_sz_%s-fltr_dim_%d-%s-%s-lambda_ABp_%s_%s_%s-c_%s-n_critic_%s" % \
        ...
        self.lambda_pair, self.clamp, self.n_critic) '''
    ...
    ###    define place holders
        ...
        self.real_PB = tf.placeholder(tf.float32,
                                        ...
                                        name='input_images_of_B_network')
        ###  define graphs
        self.r_A = tf.placeholder(tf.float32,
                                        [self.batch_size, self.image_size, self.image_size,
                                         self.input_channels_A ],
                                        name='input_images_of_A_network')
        self.r_B = tf.placeholder(tf.float32,
                                        [self.batch_size, self.image_size, self.image_size,
                                         self.input_channels_B ],
                                        name='input_images_of_B_network')
        #with tf.device('/gpu:0'):
        self.translated_PA = self.A_g_net(self.real_PA, reuse = False)
        self.translated_PB = self.B_g_net(self.real_PB, reuse = False)
        self.predictions_PAB_pair = self.C_d_net(tf.concat( [self.real_PA,self.real_PB],3), reuse = False)
        self.predictions_PA_pair = self.C_d_net(tf.concat([self.real_PA,self.translated_PA],3), reuse = True)
        self.predictions_PB_pair = self.C_d_net(tf.concat([self.translated_PB,self.real_PB],3), reuse = True)
        
        
        self.C_d_loss_fake = tf.reduce_mean(tf.square(self.predictions_PA_pair))+ \
        tf.reduce_mean(self.predictions_PB_pair)
        self.C_d_loss_real = tf.multiply(2.0,  tf.reduce_mean(tf.square(1-self.predictions_PAB_pair)) )
        self.C_d_loss = self.C_d_loss_fake + self.C_d_loss_real
        
        
        self.translated_A = self.A_g_net(self.real_A, reuse = True)
        ...
        self.translated_B = self.B_g_net(self.real_B, reuse = True)
        ...
        ### define loss
        self.trans_A = self.A_g_net(self.r_A, reuse = True)
        self.trans_B = self.B_g_net(self.r_B, reuse = True)
        ...
        if self.loss_metric == 'L1':
            ...
            self.PA_loss = tf.reduce_mean(tf.abs(self.real_PA - self.translated_PB))
            self.PB_loss = tf.reduce_mean(tf.abs(self.real_PB - self.translated_PA))
            self.loss_PAB  = tf.reduce_mean(tf.abs(self.real_PA - self.translated_PB)) + \
            tf.reduce_mean(tf.abs(self.real_PB - self.translated_PA))
            self.rA_loss = tf.reduce_mean(tf.abs(self.r_A - self.trans_B))
            self.rB_loss = tf.reduce_mean(tf.abs(self.r_B - self.trans_A))
            ...
            self.PA_loss =  tf.reduce_mean(tf.square(self.real_PB - self.translated_PA))
            self.PB_loss = tf.reduce_mean(tf.square(self.real_PA - self.translated_PB))
            self.loss_PAB = tf.reduce_mean(tf.square(self.real_PB - self.translated_PA)) +\
            tf.reduce_mean(tf.square(self.real_PA - self.translated_PB))
        
        
        self.A_d_loss_real = tf.reduce_mean(tf.square(1-self.A_D_predictions_))
        self.A_d_loss_fake = tf.reduce_mean(tf.square(self.A_D_predictions) )# + tf.reduce_mean(self.A_D_predictions_2)
        
        self.A_d_loss = self.A_d_loss_fake + self.A_d_loss_real
        self.A_g_loss = tf.reduce_mean(tf.square(1-self.A_D_predictions)) + self.lambda_B * (self.B_loss )

        
        self.B_d_loss_real = tf.reduce_mean(tf.square(1-self.B_D_predictions_))
        self.B_d_loss_fake = tf.reduce_mean(tf.square(self.B_D_predictions)) #+ tf.reduce_mean(self.B_D_predictions_2)
        
        self.B_d_loss = self.B_d_loss_fake + self.B_d_loss_real
        self.B_g_loss = tf.reduce_mean(tf.square(1-self.B_D_predictions)) + self.lambda_A * (self.A_loss )
        ...
        #self.g_loss_pair = self.loss_PAB 
        self.g_loss_pair = self.A_g_loss+self.B_g_loss
        
        if self.use_labeled_data:
            #L1 :self.g_loss = self.loss_PAB
            
            #cGAN :self.g_loss = self.g_loss_pair 
            #L1+GAN:
            #self.d_loss = self.A_d_loss+self.B_d_loss
            self.g_loss = self.loss_PAB
        ...
        self.translated_A_sum = tf.summary.image("real_PA", self.translated_PA)
        self.translated_A_sum = tf.summary.image("translated_PA", self.translated_PA)
        ...
        self.C_d_loss_sum = tf.summary.scalar("C_d_loss", self.C_d_loss)
        self.PA_loss_sum = tf.summary.scalar("PA_loss", self.PA_loss)
        self.PB_loss_sum = tf.summary.scalar("PB_loss", self.PB_loss)
        self.A_g_loss_sum = tf.summary.scalar("A_g_loss", self.A_g_loss)
        self.B_g_loss_sum = tf.summary.scalar("B_g_loss", self.B_g_loss)
        self.g_loss_pair_sum = tf.summary.scalar("g_loss_pair", self.loss_PAB)
        self.rA_loss_sum = tf.summary.scalar("rA_loss", self.rA_loss)
        self.rB_loss_sum = tf.summary.scalar("rB_loss", self.rB_loss)
        ...
        #self.d_loss_sum1 = tf.summary.merge([self.C_d_loss_sum])
        self.g_loss_sum1 = tf.summary.merge([self.g_loss_pair_sum, self.PA_loss_sum, self.PB_loss_sum])
        #self.merged=tf.merge_all_summaries()
        self.g_loss_sum2 = tf.summary.merge([self.rA_loss_sum, self.rB_loss_sum])
        ...
        '''if  self.use_labeled_data:
            self.d_vars = self.C_d_vars
        else:
            self.d_vars = self.A_d_vars + self.B_d_vars'''
        if not self.use_labeled_data:
            self.d_vars = self.A_d_vars + self.B_d_vars
    ...
    def load_random_samples(self):
        #np.random.choice(
        data =np.random.choice(glob('./datasets/{}/val/A2/*.jpg'.format(self.dataset_name)),self.batch_size)
        sample_A = [load_data(sample_file, image_size =self.image_size, flip = False) for sample_file in data]
        
        data = np.random.choice(glob('./datasets/{}/val/B2/*.jpg'.format(self.dataset_name)),self.batch_size)
    ...
    def load_val_samples(self):
        batch_files =np.random.choice(glob('./datasets/{}/val/AB/*.png'.format(self.dataset_name)),self.batch_size)
    ...
    def sample_shotcut(self, sample_dir, epoch, idx, batch_idxs):
        ...
        save_images(sample_A_imgs, [self.batch_size,1], './{}/{}/{:06d}_{:04d}_train_A_{:02d}.png'.format(sample_dir,self.dir_name , epoch, idx, batch_idxs))
        ...
        save_images(sample_B_imgs, [self.batch_size,1], './{}/{}/{:06d}_{:04d}_train_B_{:02d}.png'.format(sample_dir,self.dir_name , epoch, idx, batch_idxs))
    ...
    def train(self, args):
        ...
        self.g_optim = tf.train.AdamOptimizer(args.lr) \
        ...
        for epoch in xrange(args.epoch):
            ...
            if self.use_labeled_data:
                data_PAB = glob('./datasets/{}/train/AB1/*.png'.format(self.dataset_name))
                #data_PAB = glob('./datasets/{}/train/C/*.jpg'.format(self.dataset_name))
                np.random.shuffle(data_PAB)
                batch_idxs = len(data_PAB)// (self.batch_size*self.n_critic)
                print(batch_idxs)
            else:
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if self.use_labeled_data:
                    imgA_batch_list = []
                    imgB_batch_list = []
                    imgPAB_batch_list = self.load_pair_imgs(data_PAB, idx, batch_idxs) 
                    #imgPAB_batch_list = [self.load_training_imgs(data_PAB, idx+i) for i in xrange(self.n_critic)]
                else:
                    imgA_batch_list = self.load_training_imgs(data_A, idx) 
                    imgB_batch_list = self.load_training_imgs(data_B, idx)
                    imgPAB_batch_list = []
        ...
        print(counter)  
    def val_loss(self,counter):
        
        batch_AB_images= self.load_val_samples()
        summary_str1= self.sess.run(self.g_loss_sum2, feed_dict={ self.r_A:batch_AB_images[:,:,:,0:self.input_channels_A],\
                self.r_B:batch_AB_images[:,:,:,self.input_channels_A:]})
    ...
    def run_optim(self,imgA_batch_list, imgB_batch_list,imgPAB_batch_list,  counter, start_time):
        #for i in xrange(self.n_critic):
            ...
            if self.use_labeled_data:
                batch_PAB_images = imgPAB_batch_list
                _,  PA_loss,  PB_loss,summary_str\
                = self.sess.run([self.g_optim,  \
                self.PA_loss,self.PB_loss,self.g_loss_sum1],\
                feed_dict={ self.real_PA:batch_PAB_images[:,:,:,0:self.input_channels_A],\
                self.real_PB:batch_PAB_images[:,:,:,self.input_channels_A:]})
                '''_,  PA_loss,  PB_loss,summary_str\
                = self.sess.run([self.g_optim,  \
                self.PA_loss,self.PB_loss,self.g_loss_sum1],\
                feed_dict={ self.real_PA:batch_PAB_images,\
                self.real_PB:batch_PAB_images})'''
                self.writer.add_summary(summary_str, counter)
            else:
                batch_A_images = imgA_batch_list
                batch_B_images = imgB_batch_list
                _,summary_str = \
                        self.sess.run([self.d_optim,self.d_loss_sum], \
                        feed_dict = {self.real_A: batch_A_images, self.real_B: batch_B_images})
                self.writer.add_summary(summary_str, counter)
                #self.sess.run(self.clip_d_vars_ops)
                _,summary_str= \
                self.sess.run([self.g_optim, self.g_loss_sum], feed_dict={ self.real_A: batch_A_images, \
                              self.real_B: batch_B_images})
                self.writer.add_summary(summary_str, counter)
        
                '''if self.use_labeled_data:
                    batch_PAB_images = imgPAB_batch_list[np.random.randint(self.n_critic, size=1)[0]]
                    _,  gloss_pair,  PAB_loss,summary_str\
                    = self.sess.run([self.g_optim,  \
                                 self.g_loss_pair, self.loss_PAB,self.g_loss_sum1],\
                                 feed_dict={ \
    ...
    def test_domain(self, args, test_log, type = 'A'):
        ...
        if type == 'A':
            ...
            for i in xrange(0, len(sample_images), self.batch_size):
                ...
                translated_A_value, recover_A_value = self.sess.run(
